...
const TitleEditForm = (id, text) => {
  return `
    <form class="js-edit-form edit-form">
      <input type="text" value=${text} data-item-id=${id}>
      <button type="submit">Save</button>
    </form>
  `;
};

const handleEditItemTitleClicked = function () {
  $('.js-shopping-list').on('click', '.js-item-edit', function (event) {
    const id = getItemIdFromElement(event.currentTarget);
    const clickedItem = $(this).parents().find(`li[data-item-id=${id}]`);
    const itemText = clickedItem.find('span').text();
    $(this).parent().find('').attr('disabled', true);
    $(this).attr('disabled', true);
    clickedItem.find('span').html(TitleEditForm(id, itemText));
    return $('.js-edit-form').on('submit', handleEditItemTitleSubmit);
  });
};

const handleEditItemTitleSubmit = function (e) {
  e.preventDefault();
  const id = $(this).find('input').data('itemId');
  const value = $(this).find('input').val();
  store.items.find((item) => item.id === id).name = value;
  return render();
};
